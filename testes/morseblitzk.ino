...
void loop() {
  ...
  if (millis() - tempoUltimaPressao > tempoEspaco && mensagemMorse.length() > 0) {
    ...
    if (letra != "") {
      ...
      lcd_1.print("Palavra: ");
      lcd_1.setCursor(0, 1);
      lcd_1.print(palavra);  // Exibe a palavra completa no LCD
  ...
  if (millis() - tempoUltimaPressao > tempoEspaco && palavra.length() > 0) {
    // Adiciona um espaço para separar as palavras
    palavra += " ";
    
    // Exibe a palavra completa com a separação no LCD
    lcd_1.clear();
    lcd_1.setCursor(0, 0);
    lcd_1.print("recado: ");
    lcd_1.setCursor(0, 1);
