...
void loop() {
  ...
  if (leituraBotao == HIGH) {  // Se o botão está pressionado
    ...
    if (botaoPressionado) {  // Se o botão estava pressionado anteriormente
      ...
      delay(300);  // Atraso entre os cliques para evitar múltiplos registros em uma única pressão
    }
  }

  // Se houver uma pausa longa entre os cliques, envia um espaço (separação de caracteres ou palavras)
  if (millis() - tempoUltimaPressao > 1500 && mensagemMorse.length() > 0) {
    Serial.println(" ");  // Espaço entre caracteres
